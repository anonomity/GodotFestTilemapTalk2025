...
func save_everything():
	...
	save_tile_layer()
	save_scene_layer()
	ResourceSaver.save(tile_save, "user://Dictionary_Method4.tres")

func load_everything():
	tile_save = ResourceLoader.load("user://Dictionary_Method4.tres")
	load_scene_tiles()
	load_tiles()
	load_terrains()
...
func load_terrains():
	if tile_save:
		for key in tile_save.terrain_dict:
			...
			tile_map_layer.set_cells_terrain_connect(cell_arr, terrain_vec.x, terrain_vec.y,false)
